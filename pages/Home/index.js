...
const Page = () => {
  ...
  if (!data) {
    // Données non disponibles, afficher un indicateur de chargement ou gérer le cas
    return <div>Loading...</div>;
  }

  if (error) {
    // Gérer l'erreur si elle est présente
...
if (!Array.isArray(events) || events.length === 0) {
  // Gérer le cas où events n'est pas un tableau ou s'il est vide
  return <div>No event data available</div>;
}

const lastProject = events[events.length - 1]; // Récupérer le dernier projet du tableau events
